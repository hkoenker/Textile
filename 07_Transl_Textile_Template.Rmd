...
params: 
  ...
  netfile: TZHR81_netfile.dta # default
...
levels(df$region) <- gsub('[^ -~]', 'e', levels(df$region), useBytes = TRUE) 
...
survey_design <-  df %>%
  as_survey_design(ids = cluster, strata = region, weights = wt)
...
pnet <- function(column) {
  ...
  survey_design %>%
    ...
    geom_text(aes(label = m, vjust = -0.5)) + # , size=3 for Tanzania (31 regions)
    labs(
      y =  "",
      x =  "",
      fill =  "",
...
# Facet Version for Tanzania 2022 DHS 
# survey_design %>%
#   group_by(region, textile) %>%
#   summarise(m = round(survey_mean(netu) * 100, 1)) %>%
#   arrange(region) %>% 
...
#     position = position_dodge(width = 0.9)
#   ) +
#   geom_text(aes(y=5, x=textile, label=m), position=position_dodge(width=0.9), size=2) +
#   labs(y =  "",
#        x =  "",
#        fill =  "",
#        title =  "Percentage of nets used") +
...
dm <- df %>%
  select(netu, textile, netsupply, month, region, residence, ses, netagegr, cluster, hhid) %>%
...
# dm %>%
#   select(netu, textile, supply, month, region, residence, ses, netagegr) %>%
#   tbl_uvregression(
#     method = glm,
#     y = netu,
...
# Multivariate
dsvy <- svydesign(ids=~cluster, weights=~wt, strata=~region, data = df)
m1 <- svyglm(netu ~ textile + netsupply + month + region + residence + ses + netagegr, design=dsvy, family = quasibinomial)
...
2. Déterminer si les données existantes sont suffisantes pour évaluer l'impact du textile sur l'utilisation des MII
    - évaluer si les deux textiles sont suffisamment représentés dans la base des données
