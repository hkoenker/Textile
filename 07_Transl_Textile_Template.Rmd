...
ob %>% 
  group_by(hv270) %>% 
  summarize(mn = survey_mean(obm)*100) %>% 
...
obmn <- obe %>% 
  summarize(obn = round(survey_mean(obe)*100,2)) %>% 
  pull()
...
<h3>Modèle multivarié</h3>
 
Ensuite, nous pouvons construire le modèle multivarié. Lors du contrôle des facteurs contextuels, le textile est-il associé à des différences significatives dans l'utilisation d'une moustiquaire ?
