...
```{r read}
df <- read_dta("BJHR71_netfile.dta") %>% 
  clean_names() %>% 
...
df <- df %>% 
  ...
  mutate(brandclean = case_when(
    grepl("dk", brand) ~ "unknown",
    grepl("other", brand) ~ "unknown",
    grepl("intercep", brand) ~ "Interceptor",
    grepl("intersep", brand) ~ "Interceptor",
    grepl("basf", brand) ~ "Interceptor",
    grepl("olys", brand) ~ "Olyset",
    grepl("olis", brand) ~ "Olyset",
    grepl("olic", brand) ~ "Olyset",
    grepl("perman", brand) ~ "PermaNet",
    grepl("premane", brand) ~ "PermaNet",
    grepl("sentry", brand) ~ "Royal Sentry",
    grepl("dct", brand) ~ "Royal Sentry",
    grepl("royal", brand) ~ "Royal Sentry",
    grepl("guard", brand) ~ "Royal Guard",
    grepl("dura", brand) ~ "Duranet",
    grepl("protec", brand) ~ "NetProtect",
    grepl("best", brand) ~ "NetProtect",
    grepl("daw", brand) ~ "DawaPlus",
    grepl("dava", brand) ~ "DawaPlus",
    grepl("tana", brand) ~ "DawaPlus",
    grepl("tuza", brand) ~ "TuzaNet",
    grepl("safe", brand) ~ "SafeNet",
    grepl("yahe", brand) ~ "Yahe",
    grepl("yorko", brand) ~ "Yorkool",
    grepl("safi", brand) ~ "SafiNet",
    grepl("serena", brand) ~ "Serena",
    grepl("tsara", brand) ~ "Tsara",
    grepl("g2", brand) ~ "Interceptor G2",
    grepl("super", brand) ~ "Super Moustiquaire",
    grepl("joia", brand) ~ "Joia",
    grepl("magnet", brand) ~ "MagNet",
    grepl("mira", brand) ~ "MiraNet",
    grepl("panda", brand) ~ "Panda Net",
    grepl("veera", brand) ~ "Veeralin",
    grepl("icon", brand) ~ "IconLife",
    grepl("lifenet", brand) ~ "LifeNet",
    grepl("life net", brand) ~ "LifeNet",
    ))
```
...
Is this missing/unknown data evenly distributed throughout the country? We can see below that in most regions, the majority of nets have a missing or unknown brand. Only in Zou and Couffo is the brand recorded for more than 75% of nets.
...
As a result, the majority of nets in Benin will also have an unknown textile. What else do you notice? Were nets of different textiles distributed evenly throughout the country? Were they concentrated in certain regions?
...
# bar chart of regression coefficients, with textile highlighted in orange, and non-significant coeffs in gray, done manually by reviewing the above table. Would be nicer to have OR with origin at 1.0. not sure. Color coding in table perhaps best of all?
